...
exports.getAll = async (req, res) => {
  try {
    ...
    let sql = 'SELECT id, title, description, category, difficulty_level, image_url, steps, views_count, completion_count FROM tutorials WHERE is_active = 1';
    const params = [];
...
exports.getById = async (req, res) => {
  try {
    const [tutorials] = await db.query(
      'SELECT id, title, description, category, difficulty_level, image_url, steps, views_count, completion_count FROM tutorials WHERE id = ? AND is_active = 1',
      [req.params.id]
    );
...
exports.incrementView = async (req, res) => {
  try {
    await db.query('UPDATE tutorials SET views_count = views_count + 1 WHERE id = ?', [req.params.id]);
